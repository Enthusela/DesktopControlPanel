...
void loop() {
  ...
  kelv = 1500;//map(analogRead(kelvPin),0,ANALOG_RANGE,DEV_LED_KELV_MIN,DEV_LED_KELV_MAX);
  valScale = 1.00;//float(map(analogRead(valScalePin),0,ANALOG_RANGE,0,100)) / 100.0;
  // Update LED devices
...
CHSV kelvToHSV(int kelv) {
  ...
  HSV.sat = map(kelv, satFullKelvLimit, DEV_LED_KELV_MAX, DEV_LED_SAT_MAX, 0);//max(kelv,satFullKelvLimit), satFullKelvLimit, DEV_LED_KELV_MAX, DEV_LED_SAT_MAX, 0);
