...
void setup() {
  ...
  delay(1000);
  //it's 2am, fuck it, hard-code the proof-of-concept for now
}
...
void loop() {
  ...
  if (millis() - devMillisLast > devRefreshTime) {
    ...
    FastLED.clear();
    if      (dot <= dot_range_pump_max) { pumpLED[dot-dot_range_pump_min] = devHSV; }
    else if (dot <= dot_range_fan1_max) { fan1LED[dot-dot_range_fan1_min] = devHSV; }
    else if (dot <= dot_range_fan2_max) { fan2LED[dot-dot_range_fan2_min] = devHSV; }
    FastLED.show();
